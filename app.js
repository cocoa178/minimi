...
function addToArchiveFromMemo(attachment, memoContent, date) {
    ...
    if (archives.some(a => a.title === attachment.title && a.date === date)) return;
...
function renderMemos() {
    ...
    container.innerHTML = filtered.map(m => {
        if (currentViewMode === 'grid') { if (!m.image) return ''; return `<div class="card-mini grid-item" onclick="viewMemo(${m.id})"><div class="card-image-placeholder"><img src="${m.image}"></div></div>`; }
...
async function searchContent() {
    ...
    try {
        const apiMap = { book: 'book.json', movie: 'movie.json', tv: 'movie.json', music: 'webkr.json', place: 'local.json' };
...
function renderEmotionInputTable() {
    ...
    for (let h = 0; h < 24; h++) {
        ...
        html += `<tr data-hour="${h}"><td class="hour-cell">${h}시</td><td class="slider-cell"><input type="range" min="1" max="5" value="${emotionValue}" class="emotion-row-slider" data-hour="${h}" oninput="updateRowEmoji(this)"><span class="row-emoji">${emojiMap[emotionValue]}</span></td><td class="reason-cell"><input type="text" class="reason-input" value="${escapeHtml(reasonValue)}" placeholder="사유" data-hour="${h}" onchange="saveEmotionRow(${h})">${existing ? `<button class="row-delete-btn" onclick="deleteEmotionByHour(${h}, '${dateStr}')">×</button>` : ''}</td></tr>`;
    }
    tbody.innerHTML = html;
}

function updateRowEmoji(slider) { slider.parentElement.querySelector('.row-emoji').textContent = emojiMap[slider.value]; saveEmotionRow(parseInt(slider.dataset.hour)); }
...
function renderEmotionGraph() {
    ...
    const days = []; for (let i = period - 1; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); days.push(d.toISOString().split('T')[0]); }
    const dailyData = days.map(day => { const dayEmotions = emotions.filter(e => e.date === day); return dayEmotions.length === 0 ? null : dayEmotions.reduce((s, e) => s + e.level, 0) / dayEmotions.length; });
