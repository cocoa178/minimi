...
function updateCurrentTimeDisplay() {
    ...
    if (el) el.textContent = `현재 ${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}`;
}
...
function showPage(page) {
    ...
    if (page === 'lab') { updateReport(); renderEmotionGraph(); renderEmotionTable(); }
}

function setViewMode(mode) {
    currentViewMode = mode;
    document.querySelectorAll('.view-icon-btn').forEach(b => b.classList.toggle('active', b.dataset.mode === mode));
    renderMemos();
}
...
function updateEmotionPreview() {
    const val = parseInt(document.getElementById('emotionSlider').value);
    document.getElementById('emotionPreview').textContent = emojiMap[val];
}

function saveEmotion() {
    const level = parseInt(document.getElementById('emotionSlider').value);
    ...
    document.getElementById('emotionSlider').value = 3;
...
function renderEmotionTable() {
    ...
    let html = '';
    for (let h = 0; h < 24; h++) {
        const hourEmotions = byHour[h] || [];
        if (hourEmotions.length === 0) {
            html += `<tr><td>${h.toString().padStart(2,'0')}:00</td><td class="emotion-cell">-</td><td>-</td><td></td></tr>`;
        } else {
            hourEmotions.forEach((e, i) => {
                html += `<tr>
                    <td>${i === 0 ? h.toString().padStart(2,'0') + ':00' : ''}</td>
                    <td class="emotion-cell">${emojiMap[e.level]}</td>
                    <td>${e.reason || '-'}</td>
                    <td><button class="delete-btn" onclick="deleteEmotion(${e.id})">×</button></td>
                </tr>`;
            });
        }
    }
...
function renderMemos() {
    ...
    // Set view mode class
    memoContainer.className = 'cards-compact';
    if (currentViewMode === 'list') memoContainer.classList.add('list-view');
    else if (currentViewMode === 'grid') memoContainer.classList.add('grid-view');
...
function renderEmotionGraph() {
    ...
    if (style === 'line') {
        const points = dailyData.map((val, i) => {
            ...
            return { x: padding.left + (i / (days.length - 1)) * graphWidth, y: padding.top + graphHeight - ((val - 1) / 4) * graphHeight, val };
        }).filter(p => p !== null);
        ...
        const barGap = graphWidth / days.length * 0.15;
...
function selectLabTab(tab) {
    ...
    if (tab === 'emotion') { renderEmotionGraph(); renderEmotionTable(); }
}
...
function generateClaudePrompt() {
    ...
    if (fEmotions.length > 0) {
        fEmotions.forEach(e => prompt += `- ${e.date} ${e.time || ''}: ${emojiMap[e.level]}${e.reason ? ' (' + e.reason + ')' : ''}\n`);
    } else prompt += `기록 없음\n`;
